...
const Link = async (interaction: CommandInteraction) => {
    ...
    if (!interaction.replied) interaction.reply({
        ...
        content: `https://osu.ppy.sh/oauth/authorize?redirect_uri=${process.env.OSUURL}&response_type=code&client_id=${process.env.OSUID}&state=${secret}&scope=${"public+identify"}`
    })
}
